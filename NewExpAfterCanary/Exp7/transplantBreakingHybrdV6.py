...
BREAKING_OUTPUT = Path("/Volumes/Rachna-HD/Exp7BatchResults/breaking/transplant_results_final_breaking_hybrid.json")
...
def _load_multi_module_info():
    ...
    if not MULTI_MODULE_LIST.exists():
        print(f"[INFO] Multi-module list not found: {MULTI_MODULE_LIST}")
        return {}
...
def main():
    ...
    print(f"\n{'='*80}")
    print(f"HYBRID Breaking Stage")
    print(f"Single-module: javac + surefire (Doc 2)")
    print(f"Multi-module: Maven reactor (Doc 1)")
    ...
    print(f"\n{'='*80}")
    print(f"✓ COMPLETE")
    print(f"Processed: {processed_count} ({single_module_count} single, {multi_module_count} multi)")
    print(f"Pass: {success_count}, Fail: {failure_count}")
    print(f"{'='*80}\n")

    # Cleanup
    if SCRATCH_BASE.exists():
        print(f"[CLEANUP] Removing: {SCRATCH_BASE}")
