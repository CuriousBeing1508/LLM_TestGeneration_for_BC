...
# This is partially wrong script as it is depending on maven failure exit code. Check v2 for corrected script.
# === CONFIG ===
...
def run_test_in_isolation(image_tag: str, custom_id: str, test_root: str, good_tests_dir: str, java_file: str):
    """
    ...
    # Copy only this java file from good_tests dir (preserving package path)
    found = None
...
def main():
    ...
    with open(CSV_PATH) as f:
        ...
        for row in reader:
            ...
            # Store per custom_id summary
            results[custom_id] = {
                "passed": per_test_status["passed"],
                "failed": per_test_status["failed"],
                "summary": {
                    "total_tests": len(per_test_status["passed"]) + len(per_test_status["failed"]),
                    "passed": len(per_test_status["passed"]),
                    "failed": len(per_test_status["failed"]),
                }
            }
    ...
    global_summary = {
        ...
        "passed": sum(r["summary"]["passed"] for r in results.values()),
        "failed": sum(r["summary"]["failed"] for r in results.values()),
    }
    ...
    print(f"[INFO] JSON summary saved to {BREAKING_OUTPUT}")
