#!/usr/bin/env python3
"""
Count tokens in the original prompt .txt files (the LLM input) using the
GPT-OSS tokenizer, across the minimal/method/class context variants. Adds an
`input_token` column to the existing OutputTokenCombined_GPTOSS.csv (produced
by getOutputTokenGPTOSS.py), joining on (context_variant, bump_id, file_name).
"""
...
MODEL = "GPT_OSS_120b"
CSV_REPORT_PATH = Path(__file__).resolve().parent / "OutputTokenCombined_GPTOSS.csv"

# Works for both gpt-oss-120b and gpt-oss-20b
...
def count_tokens(text: str) -> int:
    """Count tokens in text using the GPT-OSS tokenizer."""
    if not text:
        return 0
    try:
...
def main():
    if not CSV_REPORT_PATH.exists():
        ...
        print("[INFO]  Run getOutputTokenGPTOSS.py first.")
        return
