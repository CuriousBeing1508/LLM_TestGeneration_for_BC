...
def read_matched_files_from_csv(csv_file):
    ...
    try:
        with open(csv_file, 'r', encoding='utf-8') as f:
            ...
            for row in reader:
                # Use the full_path from CSV
                matched_files.add(os.path.normpath(row['full_path']))
...
def copy_non_matched_files(source_folder, destination_folder, matched_files):
    ...
    excluding the matched files from the CSV.
    """
    copied_count = 0
    skipped_count = 0
    total_files = 0
...
if __name__ == "__main__":
    ...
    csv_file = "/Volumes/Rachna-HD/Exp3BatchResults/test_files_filter.csv"
    
    # Source folder (the original folder with all files)
    source_folder = "/Volumes/Rachna-HD/GeneratedPromptsClientsExp3"

    # Destination folder (where non-matched files will be copied)
    destination_folder = "/Volumes/Rachna-HD/FilteredDataset/Exp3Prompts"
    # =======================================================
