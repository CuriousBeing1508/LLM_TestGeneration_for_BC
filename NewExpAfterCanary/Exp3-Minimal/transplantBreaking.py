...
PRE_RESULTS_PATH = SECONDARY_DRIVE / "Exp3BatchResults/pre/transplant_results_final_pre.json"
BREAKING_OUTPUT = SECONDARY_DRIVE / "Exp3BatchResults/breaking/transplant_results_final_breaking.json"
ABC_ROOT = SECONDARY_DRIVE / "GeneratedOutputClientsExp3/GPT4o"
MODEL_NAME = ABC_ROOT.name  # e.g., "GPT4o"

SCRATCH_BASE = SECONDARY_DRIVE / "Exp3BatchResults/breaking/scratch"
REPORTS_BASE = SECONDARY_DRIVE / "Exp3BatchResults/breaking/reports"
...
def run_test_in_isolation(image_tag: str, custom_id: str, test_root: str,
    ...
    Assumes the project is already compiled in the Docker image.
    """
    
    # [Keep all the initial code for finding source, creating scratch, etc. - same as before]
    ...
    # === KEY FIX: Don't recompile! Just compile our test with javac, then run ===
    
    if module_name and project_root and project_root != "/":
        # Multi-module: use javac to compile our test, then run with surefire
        maven_cmd = (
            f"cd {project_root}/{module_name} && "
            # Compile our test file using javac with existing classpath
            f"javac -cp \"target/classes:target/test-classes:$(find {project_root}/*/target/classes -type d 2>/dev/null | tr '\\n' ':')$(mvn dependency:build-classpath -q -DincludeScope=test -Dmdep.outputFile=/dev/stdout 2>/dev/null)\" "
        ...
        # Single module: compile our test with javac, then run
        maven_cmd = (
            f"cd {project_root} && "
    ...
    try:
        ...
        test_ran = False
        our_test_found = False
        
        if f"Running {fqn}" in combined:
            test_ran = True
            our_test_found = True
            log_lines.append(f"[INFO] ✓ Found: 'Running {fqn}'")
        elif f"Running {test_class}" in combined:
            test_ran = True
            our_test_found = True
            log_lines.append(f"[INFO] ✓ Found: 'Running {test_class}'")
        
        if test_ran and our_test_found:
            ...
            if m:
                ...
                log_lines.append(f"")
                log_lines.append(f"[RESULTS] {test_class}:")
                ...
                if tests_run > 0:
                    ...
                    if success:
                        log_lines.append(f"[PASS] ✓ Test PASSED - No breaking change detected")
                    else:
                        log_lines.append(f"[FAIL] ✗ Test FAILED - Breaking change detected!")
                else:
                    log_lines.append("[ERROR] 0 tests ran")
                    success = False
            else:
                log_lines.append("[WARN] Could not parse results")
                success = False
        else:
            log_lines.append(f"[ERROR] ✗ Test '{test_class}' was NOT executed")
            
            # Check for compilation errors
            if "error:" in combined.lower() and (".java:" in combined or "symbol:" in combined):
                log_lines.append("[DIAGNOSIS] Compilation error in test")
                # Extract error lines
                error_lines = [line for line in combined.split("\n") if "error:" in line.lower()]
                for err_line in error_lines[:3]:
                    log_lines.append(f"  → {err_line.strip()}")
            elif "BUILD FAILURE" in combined:
                log_lines.append("[DIAGNOSIS] Build failed")
            elif "No tests were executed" in combined or "No tests found" in combined:
                log_lines.append("[DIAGNOSIS] Maven could not find test")
            else:
                log_lines.append("[DIAGNOSIS] Unknown - review output")
