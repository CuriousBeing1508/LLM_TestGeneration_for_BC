...
from common import parse_package_summary, classify_compilation_error, clean_llm_code
from common import (
...
def get_optimal_workers(num_tests, max_workers=8):
    ...
    """
    if num_tests <= 2:
        return 2  # 1-2 test filess: use 2 workers
    elif num_tests <= 4:
        return 4  # 3-4 tests files: use 4 workers
    else:
        return max_workers  # 5+ tests files: use max workers (8)

_args, _paths = parse_common_args(
    "PRE Phase 1: compile LLM-generated tests against the PRE (pre-upgrade) codebase"
)

CSV_PATH = _paths["csv_path"]
SUMMARY_PATH = _paths["summary_path"]
COMPILE_OUTPUT = _paths["compile_output"]
...
def compile_test_in_docker(image_tag: str, custom_id: str, test_root: str,
    ...
    try:
        ...
        # resolving the classpath via a separate `mvn dependency:build-classpath`
        # invocation was fragile - if that step failed for any reason the classpath
        # silently became empty and javac failed with misleading "package X does not
        # exist" errors. `test-compile` reuses Maven's own dependency resolution
        # (the same mechanism `mvn test` always uses), and since the read-only mount
        # replaces test_root with only this one file, it isolates to just this file.
        compile_cmd = (
            f"cd {project_root} && "
            f"mvn test-compile "
