...
from common import (
    extract_llm_java_block as _extract_llm_java_block,
    rewrite_package_and_class as _rewrite_package_and_class,
    sanitize_class_name as _sanitize_class_name,
    to_java_filename as _to_java_filename,
)
...
# BREAKING_OUTPUT = PRIMARY_DRIVE / "GPTResults/Exp6BatchResults/bre/transplant_results_breaking_multi_module.json"
...
_args, _paths = parse_common_args(
    "BREAKING stage: execute carried-forward tests against the BREAKING (post-upgrade) codebase - multi-module projects"
)

CSV_PATH = _paths["csv_path"]
SUMMARY_PATH = _paths["summary_path"]
PRE_RESULTS_PATH = _paths["final_pre_output"]
BREAKING_OUTPUT = _paths["breaking_multi_output"]
MULTI_MODULE_LIST = _paths["multi_module_list"]
ABC_ROOT = _paths["abc_root"]
LOG_DIR_BATCH_BRE = _paths["log_dir_bre"]
...
def main():
    global success_count, failure_count, compilation_error_count, test_failure_count, transplant_issue_count, build_failure_count, timeout_count
    ...
    multi_module_carry_forward = {
    k: v for k, v in carry_forward_tests.items() 
    if k in multi_module_info and len(v.get("passed", [])) > 0
}
    ...
    try:
        with open(CSV_PATH) as f:
            ...
            for row in reader:
                ...
                safe_print(f"[DEBUG] Checking {custom_id}...")
                safe_print(f"  - In carry_forward? {custom_id in carry_forward_instances}")
                safe_print(f"  - Has ABC files? {_abc_has_any_file(custom_id)}")
                safe_print(f"  - Already in results? {custom_id in results}")

                if custom_id not in carry_forward_instances or not _abc_has_any_file(custom_id):
                    safe_print(f"[SKIP] {custom_id} - Missing carry_forward or ABC files")
                    skipped_count += 1
                    continue
                ...
                results[custom_id] = {
                    ...
                    "summary": {
                        ...
                        "timeouts": sum(1 for f in per_test_status["failed"] 
                                                if isinstance(f, dict) and f.get("result_type") == "timeout")
                    }
                }
