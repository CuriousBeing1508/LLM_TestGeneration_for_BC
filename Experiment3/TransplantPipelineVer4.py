...
RESULT_JSON_PATH = Path("/Volumes/Rachna-HD/Experiment3Results/transplant_results.json")
SUREFIRE_OUT = Path("/Volumes/Rachna-HD/Experiment3Results/surefire_reports")
...
def classify_compilation_error(log_content):
    ...
    if "lambda expressions are not supported in" in log_lower:
        ...
        return {
            ...
            "content": "Backticks (`) are not valid in Java. Possibly introduced by LLM formatting during transplant."
        }
    ...
    elif " ';' expected" in log_lower or "illegal start of type" in log_lower or "not a statement" in log_lower:
        return {
            "type": "syntax error",
...
def run_test_in_docker(image_tag, test_root, package_path, test_files_dir, test_classes, custom_id, image_type):
    ...
    if proc.returncode != 0:
        if "No tests matching pattern" in combined_output:
            raise MavenTestError("No matching tests found", log_path)
        elif "COMPILATION ERROR" in combined_output or "BUILD FAILURE" in combined_output:
            error_info = classify_compilation_error(combined_output)
            raise MavenTestError(json.dumps({"compilationError": error_info}), log_path)
        else:
            raise MavenTestError("Test run failed", log_path)

    return SUREFIRE_OUT / f"{custom_id}_{image_type}", log_path

# === Copy for Breaking Phase ===
        ...
        for row in reader:
            ...
            try:
                ...
                results[custom_id]["pre"] = {
                    "status": "error",
                    "error_message": str(e),
                    "compilationError": json.loads(str(e)).get("compilationError") if "compilationError" in str(e) else None
                }
                continue
