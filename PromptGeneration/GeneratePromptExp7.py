# Baseline prompt: pass the whole focal class for better context to the LLM.
# The goal is to generate tests that expose breaking changes in OSS libraries.
...
- CSV_PATH_IN: CSV with at least: custom_id, clientProject, previousVersion, newVersion, test_framework
...
# Prompt generator
# -------------------------
def generate_prompt_from_usage_block(block, class_name, row_entry: dict, bump_id: str):
    ...
    # Focal class source (whole file)
    focal_class_source = ""
    if file_path:
        src_path = resolve_focal_class_path(bump_id, file_path)
        if src_path and src_path.exists():
            try:
                focal_class_source = src_path.read_text(encoding="utf-8", errors="replace")
            except Exception as e:
                focal_class_source = f"// [ERROR reading {src_path}: {e}]"
        else:
            focal_class_source = f"// [ERROR: could not locate focal class source for filePath='{file_path}']"
...
def process_bump_instance(bump_dir: Path, by_custom_id: dict):
    ...
    for idx, usage_block in enumerate(usage_blocks):
        ...
        prompt = generate_prompt_from_usage_block(usage_block, class_name, row_entry, bump_dir.name)
        if prompt:
