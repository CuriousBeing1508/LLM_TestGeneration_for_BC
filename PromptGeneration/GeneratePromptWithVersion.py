...
OUTPUT_ROOT = ROOT_DIR.parent / "GeneratedPromptsWithVersion"
...
def generate_prompt_from_usage_block(block, class_name):
    ...
    prompt_lines = [
        ...
        f"This test is based on usage from project `{client_project}`, using the library package `{package_prefix}` (version `{library_version}`).",
...
def process_bump_instance(bump_dir):
    ...
    for idx, usage_block in enumerate(usage_blocks):
        ...
        if prompt:
            ...
            print(f"⏭️ Skipping {json_file.name}, block {idx}: No method_call usage")
